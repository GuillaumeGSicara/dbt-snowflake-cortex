...
def call_snowflake_endpoint(
    ...
    Args:
        method (str): Supported REST API method (GET, POST, PUT, DELETE, PATCH, OPTIONS, HEAD).
        endpoint (str): The Snowflake endpoint to call.
        env (str): The environment to run the request in ('prod' or 'dev').
        request_body (Dict[str, Any], optional): Body of the request. Defaults to {}.
    ...
    if env == "prod":
        ...
        if host is None or token is None:
            raise ValueError("Both 'host' and 'token' must be provided in the 'dev' environment.")
        return _call_snowflake_dev(method, endpoint, host, token, request_body)
    else:
...
def _call_snowflake_prod(
    ...
    """Helper function to call Snowflake endpoint in production environment."""
    import _snowflake
