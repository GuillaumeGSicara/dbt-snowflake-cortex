...
from constants import TAGS, COUNTRIES, CAMPAIGNS, PRODUCTS
...
def generate_random_probability_distribution(n: int) -> list[float]:
    """
...
def generate_sales() -> list[dict[str, Any]]:
    ...
    for customer_id in range(NB_CUSTOMERS):
        ...
        for rand_number_of_purchase in range(number_of_purchase):
            sales_data.append(
                {
                    "purchase_id": str(customer_id).zfill(9) + str(rand_number_of_purchase).zfill(9),
                    "customer_id": customer_id,
